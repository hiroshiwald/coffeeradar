...
### 2026-05-12 — Strip HTML before running note heuristics
- **Symptom**: many Shopify feeds (Theory, ReAnimator, Black & White, Code Black, Olympia, etc.) showed no tasting notes on the public table, even when the feed clearly carried a `Tasting Notes:` block in its `<summary>`. Type, process, price, and image all extracted correctly — only notes were empty.
- **Root cause**: Shopify CDATA wraps every field label in `<strong>…</strong>`. After XML parsing the heuristic layer sees text like `<strong>Tasting Notes:</strong> Candied Lemon, Rose, Jasmine, Raspberry<br>`. The note-capture regex in `src/lib/heuristicsData.ts` (`/(?:tasting\s*)?notes?\s*(?:of|:|-)\s*([^.<\n]+)/gi`) deliberately excludes `<` so it cannot bleed across HTML tags — but the very next character after `:` is `<` (start of `</strong>`), so the `([^.<\n]+)` capture group never matches and the whole pattern fails. Confirmed against a verbatim Theory atom feed.
- **Changed**: `src/lib/feedParserHelpers.ts` — added `stripHtml(s)` (3-line pure helper: tags → space, collapse whitespace, trim). `src/lib/feedParser.ts` — wrapped the `allText` assignment in both `parseAtomFeed` and `parseRssFeed` with `stripHtml(...)`. Heuristics now operate on visible prose only.
- **Why this and not Jules's three remaining changes**: the bug is upstream of `filterNoiseTokens`, `collectNotesFromPatterns`, and `NOTE_PATTERNS`. Each of those changes would still hit the `<` boundary and short-circuit. HTML stripping obsoletes that whole line of work.
- **Untouched**: `extractImage` still operates on the raw entry object, so the `<img>` discovery path in `extractImageFromHtml` is unaffected. No changes to `heuristics.ts`, `heuristicsData.ts`, or `NOTE_WORDS`.
- **Tests**: `npm test` 195/195 (was 189). Added 5 unit tests for `stripHtml` and one integration test in `feedParser.test.ts` that runs a verbatim Theory `<entry>` (captured in `src/lib/__tests__/fixtures/realFeedSamples.ts`) through `parseAtomFeed` and asserts `Lemon`, `Rose`, `Jasmine`, `Raspberry`. `npx tsc --noEmit` clean.
- **Not fixed by this change**: feeds whose notes header has no `:`/`-` after it (e.g. Theory's `Dark Side of the Bloom` and `Tres Fuertes` use a bare `Flavor Notes\n…` followed by a newline). Accepted as a known limitation — 9 of 11 sampled Theory entries are recovered by this change, the remaining 2 fail for a separate orthogonal reason and don't justify further heuristic work without more evidence.

### 2026-05-12 — Revert PR #67 (broke 139 feeds)
- **Reverted commit**: `7a70316` (merged via PR #67 / `c1b31c6`). New revert commit on branch `claude/fix-broken-feeds-ebdj7`.
- **Why**: 139 feeds began failing after that change shipped. Dominant cause was the HTML scraping fallback added inside `fetchOne()` in `src/lib/feedFetcher.ts`: up to 5 extra outbound HTTP requests per feed per refresh, multiplied by `FEED_CONCURRENCY` feeds in flight. Bursty fan-out against shared Shopify CDN hosts pushed total per-feed work past upstream timeouts and triggered rate limiting that took out the parent feed fetch too. Secondary issues: `filterNoiseTokens` upper bound bumped 30→150 let long noise phrases through as "notes"; `collectNotesFromPatterns` rewritten as an O(segments × NOTE_WORDS) regex sweep with looser match semantics; and the `/similar\s*to\s*([^.<\n]+)/gi` pattern in `heuristicsData.ts` swallowed whole sentences.
- **Files restored**: `src/lib/feedFetcher.ts`, `src/lib/heuristics.ts`, `src/lib/heuristicsData.ts`, plus the three test files Jules retouched, README.md, and the prior DEVLOG entry (the 2026-05-11 entry above was added in the same commit and removed by the revert).
- **Test state after revert**: 187/189 passing. The 2 remaining failures (`heuristics.test.ts` "extracts subword flavors inside an explicit segment", `feedParser.test.ts` "extracts notes from description") are **pre-existing on `main`** — documented in the 2026-04-08 entry above — and were the real bug Jules was trying to fix. They are unrelated to the 139 feed failures.
- **Gotcha for the next attempt**: do not add per-entry product-page scraping inside `fetchOne`. If a Methodical-style fix is needed it must be (a) out of band from the feed fetch path, (b) per-host rate-limited, and (c) bounded in total wall time so it cannot extend a refresh cycle.

### 2026-09-11 — Make roaster renames safe in cleanDuplicateCoffees
- **Problem**: `buildStableId()` in `src/lib/feedParser.ts:21` hashes `roaster.trim().toLowerCase()` into every coffee id. `upsertCoffees` resolves conflicts on `id`, so renaming a feed source gives all of its coffees new ids. They insert as new rows and the old rows survive. `cleanDuplicateCoffees()` grouped on `(roaster, coffee, link, date)`, so the two rows landed in different groups and neither was removed. The same coffee stayed listed twice under two roaster names until it aged out on the 30-day cutoff.
- **Changed**: `src/lib/db.ts` — extracted the statement as exported `DEDUPE_COFFEES_SQL` and changed `GROUP BY roaster, coffee, link, date` to `GROUP BY coffee, link, date`.
- **Why dropping `roaster` is safe**: `link` is the product URL on the roaster's own domain, and `resolveAtomLink`/`resolveRssLink` fall back to the source's `website`, so it is never empty. No two distinct roasters in `data/sources.json` share a domain (verified: 0 duplicate hostnames outside the two same-roaster pairs). Two different roasters therefore cannot produce the same `link`.
- **Effect**: the stale row is removed on the next `cleanOldData()` run, which happens on cron and on manual refresh. Renames are now safe permanently, not just for one planned cleanup.
- **Tests**: `src/lib/__tests__/db.test.ts` — 4 new cases run `DEDUPE_COFFEES_SQL` against a real in-memory SQLite (`createClient({ url: "file::memory:" })`) rather than asserting on the SQL text: rename duplicate is removed, same coffee name across two roasters is kept, same coffee on two dates is kept, clean table is a no-op. Confirmed all 4 describe behaviour the old query got wrong. `npm test` 199/199 (was 195). `npx tsc --noEmit` clean.
- **Gotcha**: the test imports `createClient` from `@libsql/client`, not `@libsql/client/web`. The `/web` build is HTTP-only and rejects `file:` URLs, so it cannot open an in-memory database.

### 2026-09-11 — Footer component with optional tip link
- **Changed**: `src/components/CoffeeTable.tsx` — replaced the bare `<p>CoffeeRadar v1</p>` at the bottom with `<Footer />`.
- **Added**: `src/components/Footer.tsx` (26 lines) — renders the version line always, and a tip sentence only when `NEXT_PUBLIC_TIP_URL` holds a valid https URL. Markup follows section 1b of the design handoff. No new Tailwind colors; all classes already appear elsewhere in the app.
- **Added**: `src/lib/tipUrl.ts` — `getTipUrl(raw: string | undefined): string | null`. Takes the raw value as an explicit argument rather than reading `process.env` in a default parameter, so the function stays pure and the env read sits at the component edge.
- **Why the `try/catch` is not a swallowed error**: `new URL()` throws on any unparseable string, and unparseable is precisely the case to reject. Comment in the file states this.
- **Added**: `src/lib/__tests__/tipUrl.test.ts` — 6 cases: https passes; http, `javascript:`, unparseable text, `undefined`, and empty string all return null.
- **Changed**: `.env.example` and `README.md` — documented `NEXT_PUBLIC_TIP_URL`, plus `CRON_SECRET`, which `src/app/api/cron/route.ts:11` reads but neither file mentioned.
- **Gotcha**: the literal text `process.env.NEXT_PUBLIC_TIP_URL` must stay inside `Footer.tsx`. Next.js inlines `NEXT_PUBLIC_*` vars into the client bundle by string-replacing that exact expression at build time, so passing the name through a variable or destructuring it would yield `undefined` in the browser.
- **Verified**: dev server with the var unset renders the version line alone; with `https://ko-fi.com/coffeeradar` it renders the full sentence and link. `npm test` 205/205 (was 199). `npx tsc --noEmit` clean.
- **Not done**: no render test for `Footer.tsx`. `vitest.config.ts` uses `environment: "node"` and only collects `src/**/*.test.ts`, and jsdom, happy-dom, `@testing-library/react` and `@vitejs/plugin-react` are all absent. Adding four dev dependencies to cover a 26-line presentational component is not worth it; the logic lives in `tipUrl.ts`, which is tested.

### 2026-09-11 — Roasters A–Z index, and fix the page's DB bootstrap
- **Bug fixed**: `src/app/roasters/page.tsx` imported `getFeedSources`/`hasTurso` straight from `@/lib/db` and read `data/sources.json` itself, bypassing `src/lib/sourceStore.ts`. It therefore never ran `initDb()`, and its `loadRoasters()` had no `try/catch`. On a Turso deployment where `/roasters` was the first route hit in a cold process, the query ran against a schema that might not exist yet and the throw surfaced as an uncaught 500. The `if (sources.length > 0)` guard only covered an empty result, not an error. The page now calls `listEnabledMasterSources()`, which memoizes `initDb()` through `ensureInit()`.
- **Changed**: `src/app/roasters/page.tsx` — down to 18 lines. Keeps the auth guard and `force-dynamic`, drops `loadRoasters()` and the `seedSources` import, hands sources to `<RoasterIndex />`. No `try/catch` added on purpose: a real DB failure should surface, per the fail-fast rule in CLAUDE.md.
- **Added**: `src/lib/roasterSummary.ts` — `summarizeRoasters()`, `countActive()`, `groupLetter()`, `groupByLetter()`. Pure, no React.
- **Added**: `src/components/RoasterIndex.tsx` (client), `src/components/roasters/LetterGroup.tsx`, `src/components/roasters/RoasterPanel.tsx`. Split three ways to keep the components small. Markup follows section 4a of the design handoff.
- **Added**: `formatHostname()` to `src/lib/formatters.ts`, replacing the unguarded inline `new URL(r.website).hostname` the old page used. Returns `""` rather than throwing during render.
- **`RoasterSummary` carries `url`, not just `name`**: two sources can share a name (Blueprint Coffee and Oddly Correct are each registered twice with different feed URLs). Keying React children and the open-panel state on `name` would give duplicate keys and open both panels on one click. `url` is unique across all 310 entries. Verified in the browser: clicking one of the two Blueprint entries opens exactly one panel.
- **`active` does no date arithmetic**: `fetchAllFeeds()` and `getCoffees()` already cut everything older than 30 days, so "released something in the last 30 days" is the same set as "has any coffees". `countActive` is `count > 0`. Comment in the file says why, so nobody adds dead date-comparison code later.
- **Memoized**: `summarizeRoasters` joins 310 sources against every coffee and sorts each match. Without `useMemo` it reran on every panel open and close. This repo already reverted table virtualization for performance, so the bar is real.
- **Kept the A–Z jump nav** that prototype section 4a has and the handoff spec dropped. 310 names on one page need it. `#` is not usable as a URL fragment, so `letterAnchorId()` maps it to `letter-num`.
- **Deviation from the handoff**: the handoff said to hardcode the "Suggest a roaster" address, with the owner supplying it. No address was supplied, so the link reads `NEXT_PUBLIC_SUGGEST_EMAIL` and is hidden when unset, matching the tip-link pattern from the previous entry. This avoids shipping either an invented address or a dead link. Swap it for a literal if preferred.
- **Tests**: `src/lib/__tests__/roasterSummary.test.ts` (19 cases) and 5 new `formatHostname` cases. `npm test` 229/229 (was 205). `npx tsc --noEmit` clean. `npm run build` clean; `/roasters` is 14 kB, 101 kB first load.
- **Browser verification**: drove the page with Playwright against a stubbed `/api/coffees` (all 310 real feeds are unreachable from this sandbox, so the API falls back to 5 demo coffees under a roaster that is not in the source list). 19 checks pass: subtitle count, merch excluded, case-insensitive join, newest-first ordering, open/close by name and by the close control, one panel for a duplicate name, inactive names render as outbound links, letter nav scrolls, no horizontal scroll at 1280px or 400px, dark mode. Playwright was installed in a scratch directory, not added to `package.json`.
- **Gotcha**: `/opt/pw-browsers` carries chromium build 1194 but a fresh `npm install playwright` expects 1243. Launch with `executablePath: "/opt/pw-browsers/chromium-1194/chrome-linux/chrome"` instead of running `playwright install`.
- **Gotcha**: the roasters page renders `…` for the active count during SSR because `useCoffeeData` fetches client-side. Any browser assertion has to wait for that to resolve; `waitUntil: "networkidle"` alone is not enough.
- **Docs**: `MANIFEST.md` and `README.md` had drifted. Both now list `/roasters`, the new components, and `formatters.ts`'s full export set (it listed only `timeAgo()` while also exporting `formatDate()`). `useCoffeeData` was already listed in a later section of both files, so the entries added here were removed again to avoid duplicates.

### 2026-09-11 — Roaster name cleanup in data/sources.json
- **Changed**: `data/sources.json` — 32 `name` fields. No `url` or `website` touched (verified by diffing both lists before and after). Still 310 entries; diff is 32 insertions, 32 deletions, every line a `"name"` key.
- **Breakdown**: 18 case-only renames (the lower-cased name is unchanged, so `buildStableId` produces the same coffee ids and rows update in place) and 14 that change the words (new ids; the old rows now clear on the next `cleanOldData()` run thanks to the dedupe fix in the earlier entry today).
- **Held back by the owner, deliberately kept as-is**: `SEY COFFEE`, `GLITCH COFFEE & ROASTERS`, `ONIBUS COFFEE`, `PERC COFFEE`, `SUMP COFFEE` are real brand stylings, not scrape artifacts. `goodboybob` also kept.
- **Not done, per the handoff's instruction**: nothing decoded for `&amp;`. The file contains zero HTML entities (`grep -c "&amp;"` returns 0). The 13 raw `&` characters remaining in names are correct and were left alone (there were 14 before the `Coffee, Matcha, & More - Academic Coffee` rename dropped one).
- **Corrections to the handoff's rename list**: it named 6 storefront cases; there are 10. It missed `Our Coffees` (the roaster is Ally Coffee; the name was a bare page heading), `Shop – Handlebar Coffee Roasters` and `Tuotteet – Slurp` (both use an en dash, so a `" - "` rule misses them), and `Tony's Coffee - Online Store`. Its blanket "all-caps to title case" rule would also have damaged the five brand stylings above, and its "contains Shop" rule would have flagged `Macondo Coffee Shop` and `Workshop Coffee`, which are correct.
- **Still open**: `Seth Taylor: Coffee By Design` needs an owner call on which half is the roaster. `Blueprint Coffee` and `Oddly Correct` are each registered twice with different WordPress feed endpoints and double-post every coffee; the owner removes the failing one in `/owner/feeds` using the health dot.
- **Production**: the seeded `feed_sources` table ignores later edits to this file (`seedFeedSources()` early-returns once any row exists), so these renames reach production only via `POST /api/admin/sources` with `action: "add"`, which upserts on `url` and overwrites `name`. Three traps when doing that: `website` must be resent or it is overwritten with the feed URL; the handler hardcodes `enabled: true`, so a disabled source is silently re-enabled; and the url must match byte for byte, including the trailing slash on the Sey Coffee feed.

### 2026-09-11 — Fix: guarantee feed entry links are roaster-specific
- **Regression this fixes**: the dedupe change earlier today (`GROUP BY coffee, link, date`) was justified by a comment claiming `link` is always a URL on the roaster's own domain. That claim was false, and the grouping could delete a different roaster's row.
- **Root cause**: `src/lib/feedParser.ts` RSS path used `String(item.link ?? website)`. `??` only catches `null`/`undefined`, so `<link></link>` left `link` as `""`, and an object-valued link produced `"[object Object]"` — both identical across every roaster that hit them. `resolveAtomLink` had the same hole: `String(rawLink["@_href"] ?? fallback)` yields `""` for an empty `href`. `date` degrades the same way, since `isoDate` is `""` for an unparseable `pubDate`. Two roasters with degenerate links, a same-named coffee and an unparsed date therefore landed in one group and `cleanDuplicateCoffees()` deleted one. 96 of the 310 sources are RSS.
- **Why not revert**: `buildStableId()` hashes `roaster` **and** `link`, so those same two entries already produced the same id and silently overwrote each other through `ON CONFLICT(id)`. Reverting the GROUP BY would have left that path broken. Both failures share one cause — `link` was allowed to degrade — so fixing the link fixes both, and renames stay safe.
...
An 8-lens adversarial review raised 32 findings; each went to 3 independent skeptics. 11 survived, 18 were refuted, 3 were doc claims I checked by hand. Applied below. Refutations that turned on "this file is not in `git diff origin/main..HEAD`" were discounted: PR #72 merged mid-review, which moved those files onto main and made the scope test meaningless, not the findings wrong.

- **`useCoffeeData` stored error responses as data** (root cause, found while testing the fix below). The hook checked only for 401 and then parsed any response body, so a 500 called `setData({})`. `data` became truthy with `data.coffees` undefined. Added `if (!res.ok) throw` so the existing catch keeps the last good payload. This also protects `CoffeeTable.tsx:33`, which does `data.coffees.filter(...)` and would have thrown on `{}`.
- **`RoasterIndex` reported "0 released something in the last 30 days" on a failed fetch.** Replaced the nested ternary (banned by CLAUDE.md) with `subtitle()`, which has three explicit branches: a real count, "Loading recent releases…", and "Recent releases could not be loaded." It never states a count it does not have.
- **`groupByLetter` could emit two blocks for one letter.** It compared each entry against the previous one, so any disagreement between sort order and block order opened a duplicate block — a duplicate React key and an ambiguous DOM id that the letter nav links to. Now keyed by letter in a Map, so a repeat is impossible regardless of sort order. `groupLetter` also strips diacritics, since `localeCompare` with sensitivity "base" sorts "Ámbar" among the A names while `/[A-Z]/` grouped it under "#". The test is anchored because uppercasing one character can produce two ("ß" → "SS").
- **Contrast.** Inactive roaster names were 2.54:1 in light and 4.16:1 in dark; letter headings 1.47:1 and 2.66:1; four `text-gray-500` classes had no `dark:` counterpart, leaving 12–13px text at 3.94:1. All measured, all below the 4.5:1 AA threshold. Moved to `text-gray-500 dark:text-gray-400` (4.83:1 and 7.93:1). `gray-500` is the lightest stock Tailwind grey that passes on white, so this is as close to the handoff's faint look as AA allows — the letter headings are visibly heavier than the prototype's.
- **Letter headings are now `<h2>`** rather than `<span>`. They are the landing targets for the letter nav.
- **Name buttons are proper disclosures**: `aria-expanded` plus `aria-controls` pointing at the panel's new id.
- **`RoasterIndex` was 60 lines**, over the CLAUDE.md limit, and my previous entry claimed the opposite. Extracted `IndexHeader` and `subtitle`; the component is 37 lines and the false claim is corrected above.
- **Docs**: README's suite list was missing four files; MANIFEST's `db.ts` row omitted `DEDUPE_COFFEES_SQL` and its `feedParserHelpers.ts` row omitted `resolveLink()`; the ampersand count in an earlier entry said 14 when the `Academic Coffee` rename had made it 13.

**Deliberately not changed**, refuted 3-0 with reasons I checked:
- Panel close button hit target (~21×16px, under WCAG 2.2 AA's 24×24). Re-clicking the roaster name closes the panel and is a full-width target, which satisfies SC 2.5.8's equivalence exception.
- `NEXT_PUBLIC_SUGGEST_EMAIL` unvalidated in a `mailto:` href. The scheme is pinned by the literal prefix and React escapes the attribute, so unlike `NEXT_PUBLIC_TIP_URL` — where the env value *is* the whole URL and could be `javascript:` — there is no failure class to guard.
- Three tests called non-discriminating. I had already checked each against the pre-change implementation.
...
### 2026-09-16 — Review of the roasters push (PRs #72 and #73): no fatal errors; boundary and length fixes
- **Reviewed**: every file in `c383092..bdc8fd0` and the PR #72 diff beneath it, against CLAUDE.md. Before any change: `npm test` 247/247, `npx tsc --noEmit` and `npm run build` clean. `data/sources.json` parses, 310 unique urls, no host shared by two different roasters. No fatal error found.
- **Fixed, `src/lib/feedParserHelpers.ts`**: `resolveLink` collapsed a relative href to the source website. Two products in one feed with relative links then shared a link; with the same title and date they also shared a coffee id, so the second overwrote the first on insert and the dedupe pass grouped them. A relative path now resolves against the website with `new URL(href, website)`, which keeps it on the roaster's domain and distinct per product. An absolute http(s) string is still returned byte for byte, so no existing id changes. A link element with attributes (`#text` beside `@_rel`) is read too; the old code gave it the website. Non-http(s) schemes still fall back.
- **Fixed, `src/hooks/useCoffeeData.ts`**: PR #73 rejected non-2xx responses but still called `setData(json)` before reading `json.meta`, so a 200 whose body is not the API contract was stored and then threw. New `src/lib/apiResponse.ts` `readApiResponse()` checks status and shape at the boundary, and the hook stores only what passes. The 12-second follow-up fetch had an empty catch; it now logs. The hook was 57 lines and is 47.
- **Fixed, `src/lib/feedFetcher.ts`**: `fetchAllFeeds` was 78 lines and swallowed a `getFeedHttpMeta` failure without a log. Extracted `loadHttpMeta()` (logs, then falls back to unconditional fetches) and `recentNewestFirst()`. 46 lines now, behaviour unchanged, the 11 existing cases pass untouched.
- **Docs**: README listed `feedParserHelpers.test.ts` twice; merged. MANIFEST and README gain `apiResponse.ts`. The `db.ts` comment above `DEDUPE_COFFEES_SQL` names the relative-path rule.
- **Tests**: `npm test` 258/258 (was 247). 7 new `resolveLink` cases, 3 parser integration cases including two same-titled products with different relative links keeping distinct ids, 4 `readApiResponse` cases. `npx tsc --noEmit` and `npm run build` clean.
- **Browser verification**: Playwright 1.56 (global install) against `next start` with `/api/coffees` stubbed, chromium at `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`. 14 checks pass: home renders from a valid payload; survives a 500 and a malformed 200 with 0 page errors and one logged warning each; keeps its rows when the post-refresh follow-up returns 500, and logs it. `/roasters` reports "could not be loaded" on a 500 and opens a panel with `aria-expanded` on a valid payload.
- **Migration effect**: none for feeds with absolute links, which includes all 213 Shopify `.atom` sources. A feed with relative links moves from the website to the resolved product URL, a second id change for it after PR #73, with the same 30-day duplicate window. Likely none in the current list: RSS links are absolute by convention and the WordPress and Squarespace generators emit absolute links.
- **Pre-existing, not changed** (none touched by the push): functions over 50 lines in `CoffeeTable` (133), `CoffeeTableFilters` (99), `CoffeeTableRow` (77), `LoginForm` (74), `SiteAccessControl` (53) and `handleWithDb` (51); the nested ternary at `CoffeeTable.tsx:102`; bare `console.error` in three owner-feeds hooks. `middleware` (59) carries the documented exception comment. The five components have no render-test harness, so splitting them is its own task with browser verification.
- **Gotcha**: keep the absolute pass-through in `absoluteHref` as a plain string return. Running an absolute link through `new URL().href` normalises it (trailing slash on a bare origin, percent-encoding), which would change every hashed coffee id.

### 2026-09-16 — Ko-fi tip link: home header link and owner toggle
- **Changed**: new `src/components/TipLink.tsx` (header treatment plus the `TipLinkTreatment` the admin preview reuses), `src/components/owner-feeds/TipLinkControl.tsx` and `useOwnerTipLink.ts` (Tip Link card under `SiteAccessControl`), `src/app/api/admin/tip-link/route.ts`, `src/lib/tipLink.ts` (gate and input validation), `src/lib/tipLinkStore.ts` (persistence). `CoffeeTable`, `RoasterIndex` and `Footer` now take a resolved `tipUrl`; `page.tsx` and `roasters/page.tsx` resolve it server-side. `db.ts` gains `dbGetSettings()`, `dbSetSetting()` and `ensureDbInit()`. No new dependency, no third-party script, no Ko-fi asset: the link is a plain anchor.
- **Persistence: Option A from the handoff.** Option B (env var only) deletes the admin card, which was half the ask. The `site_settings` key/value table was already in `SCHEMA_DDL` and unused, so this needed no migration. `NEXT_PUBLIC_TIP_URL` stays the default when no row exists, and the literal stays in `tipLinkStore.envDefaults()`, so a deployment that never opens the card keeps its footer line.
- **One gate, `resolveTipUrl()`**: toggle first, then `getTipUrl()`. `TipLink` and `Footer` render what they are handed and never re-check the protocol. Turning the link on with a URL that `getTipUrl()` rejects returns 400 instead of storing a setting that would look saved and show nothing.
- **Found and fixed while verifying: the tip button overflowed the page on phones.** The design keeps the button below `sm` and hides only the question text, but the header group is then 462px wide in a 390px viewport, so the whole page scrolled sideways. Measured with the link off for comparison: 390px, no scroll. The header row is now `flex-wrap` with `gap-y-3`, which drops the controls to a second line on phones and changes nothing at or above `sm`. The design prototype has no media query, so this case was never in it.
- **Gotcha: the no-Turso fallback has to live on `globalThis`.** With a module-level variable, toggling off hid the link on `/` but not on `/roasters`: the dev server compiles each route into its own module instance, so each route had its own copy. Curl across both pages caught it. With Turso both routes read the same row, so the bug only bites local runs, which is exactly where the card gets exercised.
- **Pre-existing, not fixed, needs a decision: `middleware.ts` is not in the build.** Next.js looks for it beside the `app` directory, which here is `src/`, so the root-level file is ignored. `.next/server/middleware-manifest.json` has `"middleware": {}` and the build prints no middleware line. Unauthenticated `GET /owner/feeds`, `/api/admin/sources`, `/api/admin/site-auth` and `/api/admin/tip-link` all returned 200 locally. The new route is guarded the same way its siblings are, which is to say not at all right now. The fix is `git mv middleware.ts src/middleware.ts`, but that turns on login for every visitor the moment it ships, so it is the owner's call and its own change.
- **Tests**: `npm test` 272/272 (was 258). New `tipLink.test.ts` covers the gate, including toggle-off beating a valid URL, and every rejection branch of `validateTipLinkInput`. New `tipLinkStore.test.ts` covers the env default and the write-then-read path without Turso. `npx tsc --noEmit` and `npm run build` clean.
- **Browser verification**: Playwright (global install) against `next dev`, chromium at `/opt/pw-browsers`. Header and card captured in light and dark. Toggling off removed the header link, the home footer line and the `/roasters` footer line. Switching on with an empty URL reverted the switch to Hidden and surfaced "The tip link needs an https URL before it can be turned on." At 390px the question text is hidden, the button renders, no horizontal scroll.
