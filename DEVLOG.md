...
- **Changed 2**: Added HTML fallback scraping to `src/lib/feedFetcher.ts`. If an entry has zero extracted tasting notes, it makes a lightweight HTML fetch (capped to 5 per feed) to the `link`, strips HTML tags, and runs `extractNotes` against the document body to find the missing notes.
