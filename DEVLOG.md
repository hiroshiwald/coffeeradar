...
- **Added**: `src/components/RoasterIndex.tsx` (client), `src/components/roasters/LetterGroup.tsx`, `src/components/roasters/RoasterPanel.tsx`. Split three ways to keep every function under the 50-line limit. Markup follows section 4a of the design handoff.
...
- **Not done, per the handoff's instruction**: nothing decoded for `&amp;`. The file contains zero HTML entities (`grep -c "&amp;"` returns 0). The 14 raw `&` characters in names are correct and were left alone.
